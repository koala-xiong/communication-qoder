# 部署与运维

<cite>
**本文引用的文件**
- [docker-compose.yml](file://docker-compose.yml)
- [init.sql](file://init.sql)
- [scripts/start-backend-mysql.sh](file://scripts/start-backend-mysql.sh)
- [communication-backend/Dockerfile](file://communication-backend/Dockerfile)
- [communication-frontend/Dockerfile](file://communication-frontend/Dockerfile)
- [communication-frontend/nginx.conf](file://communication-frontend/nginx.conf)
- [communication-backend/src/main/resources/application.yml](file://communication-backend/src/main/resources/application.yml)
- [communication-backend/src/main/resources/application-docker.yml](file://communication-backend/src/main/resources/application-docker.yml)
- [communication-backend/src/main/resources/application-mysql.yml](file://communication-backend/src/main/resources/application-mysql.yml)
- [communication-backend/src/main/java/com/communication/config/SecurityConfig.java](file://communication-backend/src/main/java/com/communication/config/SecurityConfig.java)
- [communication-backend/src/main/java/com/communication/config/WebConfig.java](file://communication-backend/src/main/java/com/communication/config/WebConfig.java)
- [communication-frontend/vite.config.ts](file://communication-frontend/vite.config.ts)
- [communication-frontend/package.json](file://communication-frontend/package.json)
- [README.md](file://README.md)
- [communication-backend/pom.xml](file://communication-backend/pom.xml)
</cite>

## 更新摘要
**变更内容**
- 新增MySQL启动脚本章节，详细介绍scripts/start-backend-mysql.sh的作用和使用方法
- 更新环境配置章节，说明如何防止意外使用H2内存数据库
- 增强数据库配置管理的最佳实践指导
- 补充开发环境与生产环境的数据库配置差异说明

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [环境配置与数据库管理](#环境配置与数据库管理)
7. [依赖分析](#依赖分析)
8. [性能考虑](#性能考虑)
9. [故障排除指南](#故障排除指南)
10. [结论](#结论)
11. [附录](#附录)

## 简介
本文件面向通信平台的部署与运维团队，提供从零到一的容器化部署方案，覆盖镜像构建、服务编排、前后端分离部署、环境变量与敏感信息管理、生产最佳实践（负载均衡、健康检查、日志管理）、监控与告警、故障排除与性能调优，以及备份恢复与灾难恢复策略。读者无需深入代码即可理解系统如何在容器环境中稳定运行。

## 项目结构
该仓库采用前后端分离的单仓多模块结构，配合 Docker Compose 实现一键编排：
- communication-backend：Spring Boot 后端，提供认证授权、内容管理、订阅、评论等 API，并通过 Nginx 对外暴露。
- communication-frontend：Vue 3 前端，使用 Vite 构建，打包后由 Nginx 提供静态资源与反向代理。
- docker-compose.yml：统一编排 MySQL、后端、前端三类服务，定义网络、卷、健康检查与依赖顺序。
- init.sql：初始化数据库脚本，确保首次启动时存在目标数据库。
- scripts/start-backend-mysql.sh：新增的MySQL启动脚本，专门用于确保MySQL数据库连接并防止意外使用H2内存数据库。
- 后端与前端各自提供 Dockerfile，实现多阶段构建与最小化运行时镜像。

```mermaid
graph TB
subgraph "容器编排"
DC["docker-compose.yml"]
end
subgraph "后端服务"
BE_Docker["communication-backend/Dockerfile"]
BE_AppYml["application.yml<br/>application-docker.yml<br/>application-mysql.yml"]
SecCfg["SecurityConfig.java"]
WebCfg["WebConfig.java"]
Script["start-backend-mysql.sh"]
end
subgraph "前端服务"
FE_Docker["communication-frontend/Dockerfile"]
FE_Nginx["nginx.conf"]
FE_Vite["vite.config.ts"]
end
subgraph "数据存储"
MySQL["MySQL 8.0"]
InitSQL["init.sql"]
end
DC --> MySQL
DC --> BE_Docker
DC --> FE_Docker
BE_Docker --> BE_AppYml
BE_AppYml --> MySQL
FE_Docker --> FE_Nginx
FE_Vite --> FE_Docker
MySQL --> InitSQL
FE_Nginx --> BE_Docker
Script --> BE_AppYml
```

**图表来源**
- [docker-compose.yml:1-60](file://docker-compose.yml#L1-L60)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)
- [communication-backend/Dockerfile:1-32](file://communication-backend/Dockerfile#L1-L32)
- [communication-frontend/Dockerfile:1-33](file://communication-frontend/Dockerfile#L1-L33)
- [communication-frontend/nginx.conf:1-42](file://communication-frontend/nginx.conf#L1-L42)
- [communication-backend/src/main/resources/application.yml:1-42](file://communication-backend/src/main/resources/application.yml#L1-L42)
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)
- [communication-backend/src/main/resources/application-mysql.yml:1-10](file://communication-backend/src/main/resources/application-mysql.yml#L1-L10)
- [communication-backend/src/main/java/com/communication/config/SecurityConfig.java:1-89](file://communication-backend/src/main/java/com/communication/config/SecurityConfig.java#L1-L89)
- [communication-backend/src/main/java/com/communication/config/WebConfig.java:1-20](file://communication-backend/src/main/java/com/communication/config/WebConfig.java#L1-L20)
- [init.sql:1-3](file://init.sql#L1-L3)

**章节来源**
- [README.md:1-193](file://README.md#L1-L193)
- [docker-compose.yml:1-60](file://docker-compose.yml#L1-L60)

## 核心组件
- MySQL 数据库：提供持久化存储，内置健康检查与初始化脚本，挂载卷保障数据持久化。
- 后端服务（Spring Boot）：多阶段构建，JVM 运行时，读取 Docker 环境变量进行数据库与 JWT 配置，暴露 8080 端口。
- 前端服务（Nginx）：多阶段构建，静态站点由 Nginx 提供，反向代理 /api/ 与 /uploads/ 到后端，SPA 路由回退至 index.html。
- 网络与卷：Compose 统一网络，MySQL 与上传目录挂载卷；后端通过命名网络访问 MySQL。
- **新增** MySQL启动脚本：专门用于确保使用MySQL数据库连接，防止意外使用H2内存数据库。

**章节来源**
- [docker-compose.yml:4-55](file://docker-compose.yml#L4-L55)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)
- [communication-backend/Dockerfile:1-32](file://communication-backend/Dockerfile#L1-L32)
- [communication-frontend/Dockerfile:1-33](file://communication-frontend/Dockerfile#L1-L33)
- [communication-frontend/nginx.conf:1-42](file://communication-frontend/nginx.conf#L1-L42)
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)

## 架构总览
下图展示容器化部署的整体交互：浏览器请求经 Nginx 反向代理，静态资源直接由 Nginx 返回，API 请求转发至后端，后端访问 MySQL 并读写上传目录。

```mermaid
graph TB
Browser["浏览器"] --> Nginx["Nginx(前端)"]
Nginx --> API["/api/* → 后端:8080"]
Nginx --> Uploads["/uploads/* → 后端:8080"]
Nginx --> SPA["SPA 路由回退 → index.html"]
subgraph "容器"
Nginx
API
Uploads
SPA
end
API --> Backend["后端(Spring Boot)"]
Backend --> DB["MySQL"]
Backend --> Vol["上传卷 /app/uploads"]
```

**图表来源**
- [communication-frontend/nginx.conf:11-34](file://communication-frontend/nginx.conf#L11-L34)
- [docker-compose.yml:25-55](file://docker-compose.yml#L25-L55)

## 详细组件分析

### 数据库层（MySQL）
- 镜像与版本：使用官方 MySQL 8.0，设置字符集与排序规则，启用健康检查。
- 初始化：通过挂载 init.sql 确保数据库存在；首次启动自动执行。
- 存储：挂载 mysql_data 卷，避免容器删除导致数据丢失。
- 端口映射：默认 3306 映射到宿主机，便于本地调试或外部工具访问。

**章节来源**
- [docker-compose.yml:4-23](file://docker-compose.yml#L4-L23)
- [init.sql:1-3](file://init.sql#L1-L3)

### 后端服务（Spring Boot）
- 多阶段构建：先在 Alpine JDK 中下载依赖与打包，再复制到更小的 JRE 镜像运行。
- 环境变量与配置文件：
  - application.yml：本地开发默认配置，包含数据库连接、JWT 密钥、上传路径等。
  - application-docker.yml：Docker 环境专用配置，优先读取环境变量覆盖默认值。
  - **新增** application-mysql.yml：MySQL专用配置文件，用于本地MySQL连接。
- 安全与路由：
  - SecurityConfig：禁用 CSRF，启用 CORS，基于路径的权限控制，JWT 过滤器前置。
  - WebConfig：将 /uploads/** 映射到文件系统上传目录。
- 上传与静态资源：后端通过 WebMvcConfigurer 暴露上传目录，Nginx 同时代理 /uploads/。

```mermaid
sequenceDiagram
participant C as "客户端"
participant N as "Nginx"
participant B as "后端(Spring Boot)"
participant DB as "MySQL"
C->>N : "GET /api/contents"
N->>B : "转发 /api/*"
B->>DB : "查询内容"
DB-->>B : "返回结果"
B-->>N : "JSON 响应"
N-->>C : "HTTP 200"
C->>N : "GET /uploads/avatar.png"
N->>B : "转发 /uploads/*"
B-->>N : "文件流"
N-->>C : "HTTP 200"
```

**图表来源**
- [communication-frontend/nginx.conf:11-29](file://communication-frontend/nginx.conf#L11-L29)
- [communication-backend/src/main/java/com/communication/config/SecurityConfig.java:63-81](file://communication-backend/src/main/java/com/communication/config/SecurityConfig.java#L63-L81)
- [communication-backend/src/main/java/com/communication/config/WebConfig.java:14-18](file://communication-backend/src/main/java/com/communication/config/WebConfig.java#L14-L18)

**章节来源**
- [communication-backend/Dockerfile:1-32](file://communication-backend/Dockerfile#L1-L32)
- [communication-backend/src/main/resources/application.yml:1-42](file://communication-backend/src/main/resources/application.yml#L1-L42)
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)
- [communication-backend/src/main/resources/application-mysql.yml:1-10](file://communication-backend/src/main/resources/application-mysql.yml#L1-L10)
- [communication-backend/src/main/java/com/communication/config/SecurityConfig.java:1-89](file://communication-backend/src/main/java/com/communication/config/SecurityConfig.java#L1-L89)
- [communication-backend/src/main/java/com/communication/config/WebConfig.java:1-20](file://communication-backend/src/main/java/com/communication/config/WebConfig.java#L1-L20)

### 前端服务（Nginx）
- 多阶段构建：Node 构建产物拷贝至 Nginx 镜像，移除默认配置，仅保留自定义配置。
- 反向代理：
  - /api/ → 后端 8080，透传 Host、X-Real-IP、X-Forwarded-* 等头部，支持 WebSocket。
  - /uploads/ → 后端 8080，用于直链访问上传资源。
- SPA 路由：所有未匹配路径回退到 index.html，保证前端路由正常工作。
- 缓存策略：对静态资源设置一年缓存与 immutable 标记，提升性能。

```mermaid
flowchart TD
Start(["请求进入"]) --> PathCheck{"路径类型"}
PathCheck --> |/api/*| ProxyAPI["代理到后端:8080/api/"]
PathCheck --> |/uploads/*| ProxyUpload["代理到后端:8080/uploads/"]
PathCheck --> |SPA 路由| Fallback["回退到 index.html"]
ProxyAPI --> End(["响应"])
ProxyUpload --> End
Fallback --> End
```

**图表来源**
- [communication-frontend/nginx.conf:11-34](file://communication-frontend/nginx.conf#L11-L34)

**章节来源**
- [communication-frontend/Dockerfile:1-33](file://communication-frontend/Dockerfile#L1-L33)
- [communication-frontend/nginx.conf:1-42](file://communication-frontend/nginx.conf#L1-L42)

### 服务编排与依赖关系
- 依赖顺序：MySQL 健康就绪后才启动后端；后端就绪后再启动前端。
- 端口暴露：MySQL 3306、后端 8080、前端 80。
- 卷管理：MySQL 数据卷与上传目录卷独立挂载，便于备份与扩容。

```mermaid
sequenceDiagram
participant D as "Docker Compose"
participant M as "MySQL"
participant S as "后端"
participant F as "前端"
D->>M : "启动并等待健康检查"
M-->>D : "健康就绪"
D->>S : "启动(依赖 MySQL 健康)"
S-->>D : "监听 8080 就绪"
D->>F : "启动(依赖后端就绪)"
F-->>D : "监听 80 就绪"
```

**图表来源**
- [docker-compose.yml:42-55](file://docker-compose.yml#L42-L55)

**章节来源**
- [docker-compose.yml:1-60](file://docker-compose.yml#L1-L60)

## 环境配置与数据库管理

### MySQL启动脚本详解
**新增功能**：scripts/start-backend-mysql.sh 是一个专门设计的启动脚本，用于确保后端服务始终使用MySQL数据库而非H2内存数据库。

#### 脚本功能特性
- **固定数据库配置**：强制设置 `SPRING_PROFILES_ACTIVE=mysql` 环境变量
- **自动路径导航**：自动切换到后端项目目录
- **安全启动**：防止意外使用H2内存数据库导致的数据丢失
- **便捷执行**：简化开发环境下的MySQL连接配置

#### 使用方法
```bash
# 在项目根目录执行
./scripts/start-backend-mysql.sh
```

#### 配置逻辑
脚本通过以下步骤确保正确的数据库配置：
1. 获取项目根目录路径
2. 导航到 `communication-backend` 目录
3. 设置 `SPRING_PROFILES_ACTIVE=mysql` 环境变量
4. 执行 `./mvnw spring-boot:run` 启动后端服务

### 数据库配置文件管理
系统提供了三种主要的数据库配置文件：

#### application.yml（本地开发默认）
- 使用本地MySQL实例
- 默认数据库URL指向 `jdbc:mysql://localhost:3306/communication`
- 适合本地开发和测试环境

#### application-docker.yml（Docker环境）
- 使用Docker网络中的MySQL服务
- 数据库URL指向 `jdbc:mysql://mysql:3306/communication`
- 通过环境变量注入数据库凭据

#### application-mysql.yml（MySQL专用）
- 用于本地MySQL连接
- 与Docker配置类似，但使用localhost连接
- 支持通过环境变量覆盖默认值

### 数据库配置最佳实践
- **开发环境**：使用 `application.yml` 或通过 `start-backend-mysql.sh` 启动
- **Docker环境**：使用 `application-docker.yml`，通过环境变量配置
- **生产环境**：使用Docker配置，确保数据库连接安全
- **防止H2误用**：通过脚本强制使用MySQL配置文件

```mermaid
graph LR
Dev["本地开发"] --> Script["start-backend-mysql.sh"]
Script --> MySQLCfg["application-mysql.yml"]
MySQLCfg --> MySQL["MySQL数据库"]
Docker["Docker环境"] --> DockerCfg["application-docker.yml"]
DockerCfg --> MySQL
Local["本地MySQL"] --> LocalCfg["application.yml"]
LocalCfg --> MySQL
```

**图表来源**
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)
- [communication-backend/src/main/resources/application.yml:1-42](file://communication-backend/src/main/resources/application.yml#L1-L42)
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)
- [communication-backend/src/main/resources/application-mysql.yml:1-10](file://communication-backend/src/main/resources/application-mysql.yml#L1-L10)

**章节来源**
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)
- [communication-backend/src/main/resources/application.yml:1-42](file://communication-backend/src/main/resources/application.yml#L1-L42)
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)
- [communication-backend/src/main/resources/application-mysql.yml:1-10](file://communication-backend/src/main/resources/application-mysql.yml#L1-L10)

## 依赖分析
- 后端对数据库的依赖：通过 application-docker.yml 的 JDBC URL 与凭据连接 MySQL。
- 前端对后端的依赖：通过 Nginx 反向代理访问 /api 与 /uploads。
- CORS 与安全：后端配置允许开发机地址，生产中需收紧来源。
- 上传目录：后端与 Nginx 共享 /uploads，需确保权限与路径一致。
- **新增** 启动脚本依赖：MySQL启动脚本依赖于正确的配置文件和环境变量设置。

```mermaid
graph LR
FE["前端(Nginx)"] --> API["后端(8080)"]
API --> DB["MySQL"]
API --> VOL["上传卷(/app/uploads)"]
FE --> VOL
Script["start-backend-mysql.sh"] --> API
Script --> MySQLCfg["application-mysql.yml"]
```

**图表来源**
- [communication-frontend/nginx.conf:11-29](file://communication-frontend/nginx.conf#L11-L29)
- [communication-backend/src/main/resources/application-docker.yml:36-37](file://communication-backend/src/main/resources/application-docker.yml#L36-L37)
- [communication-backend/src/main/java/com/communication/config/WebConfig.java:14-18](file://communication-backend/src/main/java/com/communication/config/WebConfig.java#L14-L18)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)

**章节来源**
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)
- [communication-backend/src/main/java/com/communication/config/SecurityConfig.java:18-26](file://communication-backend/src/main/java/com/communication/config/SecurityConfig.java#L18-L26)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)

## 性能考虑
- 静态资源缓存：Nginx 对 JS/CSS/媒体资源设置一年缓存与 immutable，显著降低带宽与延迟。
- 上传优化：/uploads/ 直接由后端提供，避免额外转发开销。
- JVM 与线程池：后端 HikariCP 连接池参数可按并发与实例数调整，Flyway 迁移仅在启动时执行。
- 前端构建：Vite 开发代理仅用于本地开发，生产镜像不包含开发服务器。
- **新增** 数据库连接优化：通过启动脚本确保稳定的MySQL连接配置，避免H2内存数据库的性能问题。

**章节来源**
- [communication-frontend/nginx.conf:36-40](file://communication-frontend/nginx.conf#L36-L40)
- [communication-backend/src/main/resources/application-docker.yml:8-11](file://communication-backend/src/main/resources/application-docker.yml#L8-L11)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)

## 故障排除指南
- 健康检查失败
  - 现象：后端/前端迟迟无法启动。
  - 排查：查看 MySQL 健康检查输出与日志；确认 JDBC URL、用户名、密码正确。
- CORS 问题
  - 现象：跨域请求被拒绝。
  - 排查：核对 SecurityConfig 中允许的来源列表；生产环境务必限制来源。
- 上传失败
  - 现象：上传成功但无法访问。
  - 排查：确认 /uploads/** 路径映射与权限；检查 WebConfig 的 upload.path 与 Nginx 代理。
- 数据库初始化
  - 现象：首次启动无表。
  - 排查：确认 init.sql 已挂载且可执行；Flyway 是否启用并完成迁移。
- **新增** H2内存数据库误用
  - 现象：应用启动但数据丢失或数据库连接异常。
  - 排查：确认使用了 `start-backend-mysql.sh` 脚本；检查 `SPRING_PROFILES_ACTIVE` 环境变量是否为 `mysql`。
- 日志定位
  - 后端日志级别已在 Docker 配置中设置；可通过容器日志查看异常堆栈与 SQL。

**章节来源**
- [docker-compose.yml:19-23](file://docker-compose.yml#L19-L23)
- [communication-backend/src/main/java/com/communication/config/SecurityConfig.java:16-26](file://communication-backend/src/main/java/com/communication/config/SecurityConfig.java#L16-L26)
- [communication-backend/src/main/java/com/communication/config/WebConfig.java:11-18](file://communication-backend/src/main/java/com/communication/config/WebConfig.java#L11-L18)
- [communication-backend/src/main/resources/application-docker.yml:22-25](file://communication-backend/src/main/resources/application-docker.yml#L22-L25)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)

## 结论
本部署方案以 Docker Compose 为核心，结合 Nginx 反向代理与 Spring Boot 的安全配置，实现了前后端分离、数据持久化与可扩展的基础能力。新增的MySQL启动脚本进一步增强了环境配置的安全性和可靠性，确保开发和生产环境中都能正确使用MySQL数据库。生产环境建议进一步完善证书、密钥管理、负载均衡与监控告警体系，以满足高可用与可观测性需求。

## 附录

### A. Docker 镜像构建与运行流程
- 后端镜像：多阶段构建，先打包后复制到运行时镜像，暴露 8080。
- 前端镜像：Node 构建产物拷贝至 Nginx，暴露 80，加载自定义 conf。
- Compose 启动：按健康检查与依赖顺序启动，端口映射与卷挂载明确。

**章节来源**
- [communication-backend/Dockerfile:1-32](file://communication-backend/Dockerfile#L1-L32)
- [communication-frontend/Dockerfile:1-33](file://communication-frontend/Dockerfile#L1-L33)
- [docker-compose.yml:25-55](file://docker-compose.yml#L25-L55)

### B. 环境变量与敏感信息管理
- 关键变量（示例清单）
  - SPRING_PROFILES_ACTIVE：切换数据库配置文件（docker/mysql）
  - SPRING_DATASOURCE_URL/USERNAME/PASSWORD：数据库连接
  - JWT_SECRET：JWT 密钥
  - UPLOAD_PATH：上传目录路径
- 管理建议
  - 使用 Docker Secret 或外部密钥管理服务注入敏感变量。
  - 生产环境禁止将密钥硬编码在镜像或配置文件中。
  - **新增** 使用启动脚本确保正确的数据库配置文件激活。

**章节来源**
- [docker-compose.yml:31-37](file://docker-compose.yml#L31-L37)
- [communication-backend/src/main/resources/application-docker.yml:32-37](file://communication-backend/src/main/resources/application-docker.yml#L32-L37)
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)

### C. 生产环境最佳实践
- 负载均衡
  - 使用反向代理（如 Nginx/Traefik）分发请求至多个后端实例。
- 健康检查
  - 后端暴露健康端点（如 Actuator），Compose 增加健康检查策略。
- 日志管理
  - 统一收集容器 stdout/stderr，集中存储与检索。
- 监控与告警
  - 指标采集（CPU/内存/连接数/响应时间），阈值告警与自动化扩缩容。
- **新增** 数据库监控
  - 监控MySQL连接池使用情况
  - 监控数据库性能指标
  - 设置数据库连接超时告警

### D. 备份恢复与灾难恢复
- 备份
  - 定期导出数据库快照；备份 MySQL 数据卷与上传卷。
- 恢复
  - 重建容器后恢复卷数据，验证数据库迁移是否成功。
- 灾难恢复
  - 多副本部署与异地容灾；演练恢复流程并记录步骤。
- **新增** 配置备份
  - 备份数据库配置文件和启动脚本
  - 确保配置变更的可追溯性

### E. 部署前检查清单
- 系统要求
  - Docker Engine 20.10+
  - Docker Compose v2+
  - 至少 2GB RAM
- 网络配置
  - 确认端口 80、8080、3306 未被占用
  - 防火墙放行相关端口
- 存储空间
  - 预留至少 1GB 磁盘空间用于数据库与日志
- 安全准备
  - 生成强 JWT 密钥
  - 配置 HTTPS 证书（生产环境）
  - **新增** 配置数据库访问权限
- 环境变量
  - 准备数据库凭据
  - 配置上传目录权限
  - **新增** 验证启动脚本可用性

### F. 常见部署场景
- 开发环境
  - 使用默认凭据和本地存储
  - 禁用 HTTPS
  - 启用详细日志
  - **新增** 使用 `start-backend-mysql.sh` 确保MySQL连接
- 测试环境
  - 使用独立数据库实例
  - 配置专门的 JWT 密钥
  - 设置合理的超时参数
- 生产环境
  - 使用外部数据库服务
  - 配置 SSL/TLS
  - 启用健康检查和监控
  - 设置自动重启策略
  - **新增** 使用Docker配置文件管理数据库连接

### G. 性能调优建议
- 数据库优化
  - 调整 innodb_buffer_pool_size
  - 配置连接池大小
  - 启用查询缓存（如适用）
  - **新增** 监控数据库连接池使用率
- 应用优化
  - 调整 JVM 堆大小
  - 配置合适的线程池
  - 启用 GZIP 压缩
- 前端优化
  - 启用 CDN 加速
  - 配置缓存策略
  - 优化图片和媒体资源
- **新增** 启动脚本优化
  - 确保启动脚本的执行权限
  - 监控脚本执行状态
  - 配置脚本错误处理机制

### H. MySQL启动脚本使用指南
#### 开发环境使用
```bash
# 启动MySQL数据库
docker-compose up -d mysql

# 使用MySQL启动脚本启动后端
./scripts/start-backend-mysql.sh

# 或者手动设置环境变量
SPRING_PROFILES_ACTIVE=mysql ./mvnw spring-boot:run
```

#### Docker环境使用
```bash
# 使用Docker Compose启动完整环境
docker-compose up -d

# 后端会自动使用Docker配置文件
# 无需手动指定数据库配置
```

#### 环境变量配置
- `SPRING_PROFILES_ACTIVE=mysql`：强制使用MySQL配置文件
- `SPRING_DATASOURCE_URL`：数据库连接URL
- `SPRING_DATASOURCE_USERNAME`：数据库用户名
- `SPRING_DATASOURCE_PASSWORD`：数据库密码

**章节来源**
- [scripts/start-backend-mysql.sh:1-9](file://scripts/start-backend-mysql.sh#L1-L9)
- [docker-compose.yml:31-37](file://docker-compose.yml#L31-L37)
- [communication-backend/src/main/resources/application-mysql.yml:1-10](file://communication-backend/src/main/resources/application-mysql.yml#L1-L10)
- [communication-backend/src/main/resources/application-docker.yml:1-43](file://communication-backend/src/main/resources/application-docker.yml#L1-L43)